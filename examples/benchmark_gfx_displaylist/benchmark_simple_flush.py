...
RUN_MODES = [True]  # Only Double Buffer mode (with swap_buffers) makes sense for DISPLAY_LIST
...
def run_suite(double_buffer_requested):
    ...
    if blit_buffer is not None:
        ...
        # Restore background to DL in retained mode for the circle tests
        # This prevents the DL engine from clearing the AABB to black,
        # ensuring the circles are drawn on top of the gradient background
        # just like in the Framebuffer benchmark.
        display.blit_buffer(0, 0, display.width, blit_height, blit_buffer)
        if active_double_buffer:
            display.swap_buffers(copy=True)
